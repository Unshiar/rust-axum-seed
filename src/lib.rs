...
mod tests {
    ...
    use super::env_handle::{get_env_db_url, get_env_host, get_env_port};

    #[test]
    fn test_get_env_db_url_default() {
        unsafe { std::env::remove_var(ENV_DB_URL_NAME) };
    ...
    fn test_get_env_db_url_custom() {
        ...
        unsafe { std::env::set_var(ENV_DB_URL_NAME, expected) };
        let url = get_env_db_url();
    ...
    fn test_get_env_host_default() {
        unsafe { std::env::remove_var(ENV_HOST_NAME) };
    ...
    fn test_get_env_host_custom() {
        unsafe { std::env::set_var(ENV_HOST_NAME, "0.0.0.0") };
        let host = get_env_host().unwrap();
    ...
    fn test_get_env_host_invalid() {
        unsafe { std::env::set_var(ENV_HOST_NAME, "invalid-host") };
    ...
    fn test_get_env_host_invalid_ipv4_format() {
        unsafe { std::env::set_var(ENV_HOST_NAME, "127.0.0.777") };
        let result = get_env_host();
        assert!(result.is_err());
    }
    ...
    fn test_get_env_port_default() {
        unsafe { std::env::remove_var(ENV_PORT_NAME) };
    ...
    fn test_get_env_port_custom() {
        unsafe { std::env::set_var(ENV_PORT_NAME, "8080") };
        let port = get_env_port().unwrap();
        assert_eq!(port, 8080);
    }
    ...
    fn test_get_env_port_invalid() {
        unsafe { std::env::set_var(ENV_PORT_NAME, "invalid-port") };
    ...
    fn test_get_env_port_invalid_negative_value() {
        unsafe { std::env::set_var(ENV_PORT_NAME, "-1111") };
        let result = get_env_port();
        assert!(result.is_err());
    }
    ...
    fn test_get_env_port_invalid_positive_value() {
        unsafe { std::env::set_var(ENV_PORT_NAME, "65 536") };
        let result = get_env_port();
        assert!(result.is_err());
    }
}
